...
contract ArianeeLost is Ownable {
  ...
  modifier onlyHasBeenMissing(uint256 _tokenId) {
    require(tokenMissingStatus[_tokenId] == true, "ArianeeLost: token status must be missing ");
    _;
  }
  ...
  modifier onlyIsNotStolen(uint256 _tokenId) {
    require(tokenStolenStatus[_tokenId] != true, "ArianeeLost: token status must be not stolen");
    _;
  }
  ...
  modifier onlyIsStolen(uint256 _tokenId) {
    require(tokenStolenStatus[_tokenId] == true, "ArianeeLost: token status must be stolen");
    _;
  }

  modifier onlyManager() {
    require(msg.sender == managerIdentity, "ArianeeLost: only manager can operate");
    _;
  }

  modifier onlyTokenOwnerOrIssuerOrAuthorizedIdentity(uint256 _tokenId) {
    require(
      smartAsset.canOperate(_tokenId, msg.sender) ||
  ...
  function setMissingStatus(
    uint256 _tokenId
  ) external onlyTokenOwnerOrIssuerOrAuthorizedIdentity(_tokenId) {
   ...
   * @param _tokenId tokenId of certificate.
   */
  function _unsetMissingStatus(
    uint256 _tokenId
  ) internal onlyIsNotStolen(_tokenId) onlyHasBeenMissing(_tokenId) {
    tokenMissingStatus[_tokenId] = false;
  ...
  function setStolenStatus(
    uint256 _tokenId
  ) external onlyIsNotStolen(_tokenId) onlyAuthorizedIdentityOrIssuer(_tokenId) {
  ...
  function unsetStolenStatus(
    uint256 _tokenId
  ) external onlyIsStolen(_tokenId) onlyAuthorizedIdentityOrIssuer(_tokenId) {
   ...
   */
  function isStolen(uint256 _tokenId) external view returns (bool _isStolen) {
   ...
   */
  function isAddressAuthorized(address _address) external view returns (bool _isAuthorized) {
